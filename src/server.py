...
class ItemService(my_service_pb2_grpc.ItemServiceServicer):
    ...
    def UpdateItem(self, request, context):
        self.cursor.execute(
            "UPDATE items SET name = ? WHERE id = ?", (request.name, request.id))
        self.conn.commit()
        return request
...
def serve():
    server = grpc.server(futures.ThreadPoolExecutor(
        max_workers=10), compression=grpc.Compression.Deflate)
    my_service_pb2_grpc.add_ItemServiceServicer_to_server(
        ItemService(), server)
